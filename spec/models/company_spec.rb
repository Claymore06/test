...
describe Company do
  ...
  describe :adress do
    context '入力値=MAXの場合、正常' do
      subject{ build(:company_1, adress: 'あ' * 100) }
      its(:valid?){ should be_true }
    end

    context '入力値=MAX+1の場合、異常' do
      subject{ build(:company_1, adress: 'あ' * (100 + 1)) }
      its(:valid?){ should be_false }
    end
  ...
  describe :url do
    context '入力値=MAXの場合、正常' do
      subject{ build(:company_1, url: 'http://aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.co.jp')}
      its(:valid?){ should be_true }
    end

    context '入力値=MAX+1の場合、異常' do
      subject{ build(:company_1, url: 'http://aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.co.jp') }
      its(:valid?){ should be_false }
    end
